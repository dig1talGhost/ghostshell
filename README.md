...
```
ghostshell-v0.0.1-linux-amd64: OK
```
Make sure the binary is executable:
```bash
chmod +x ghostshell-v0.0.1-linux-amd64
```
Move the binary in your `$GOBIN` as `ghostshell`:
```bash
mv ./ghostshell-v0.0.1-linux-amd64 "$GOBIN/ghostshell"
```
#### Using git
Clone the the latest git:

```bash
git clone https://github.com/dig1talGhost/ghostshell.git
```
